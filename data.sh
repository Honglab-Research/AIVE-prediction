...
echo -e "date\thost\tNextstrain_clade\tpango_lineage\tdeletions\tinsertions\taaSubstitutions" > data.txt

awk -F'\t' -v date="$date" -v host="$host" -v clade="$clade" -v lineage="$lineage" -v deletions="$deletions" -v insertions="$insertions" -v aa="$aa" 'NR>1{print $date"\t" $host"\t" $clade"\t" $lineage"\t" $deletions"\t" $insertions"\t" $aa}' metadata.tsv >> data.txt
